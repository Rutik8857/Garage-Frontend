...
export default function UsersPage() {
  ...
  const handleDelete = (userId) => {
    // 1. Use ConfirmContext callback pattern
    confirm("Are you sure you want to delete this user?", async () => {
      try {
        ...
        // 3. Check for errors (parse JSON error if available)
        if (!response.ok) {
          const errorData = await response.json().catch(() => ({}));
          throw new Error(errorData.message || "Failed to delete user.");
        }

        // 4. Update UI ONLY after backend success
      ...
      <div className="flex flex-col min-h-screen w-full p-4 sm:p-6 lg:p-8 bg-slate-100">
        ...
        <div className="bg-white rounded-lg shadow-md overflow-hidden">
          ...
          <div className="bg-blue-600 p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <Link
              href="/StaffManageNewEmp/layout"
              className="bg-white text-blue-600 font-semibold py-2 px-4 rounded-md shadow-sm hover:bg-gray-100 transition-colors flex items-center justify-center w-full sm:w-auto text-sm"
            >
              <PlusIcon />
              Add User
            </Link>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex items-center space-x-2">
                          <button
                            onClick={() =>
                              router.push(
                                `/StaffManageEmp/edit/${user.id}/layout`
                              )
                            }
              ...
              {users.map((user, index) => (
                ...
                >
                  ...
                  <div className="border-t pt-3 mt-2 flex items-center justify-start space-x-2">
                    <Link
                      href={`/StaffManageEmp/edit/${user.id}/layout`}
                      className="flex items-center justify-center bg-teal-500 text-white px-3 py-1 rounded-md text-xs hover:bg-teal-600"
                    >
                      <EditIcon /> Edit
                    </Link>
